...
namespace AzureUploader
{
    public class AzureFtpUploader
    {
        ...
        private readonly FtpCommandExecutor _baseFtpCommand;
        ...
        public AzureFtpUploader(Func<FtpClient> clientFactory, ILogger logger = null)
        {
            ...
            _baseFtpCommand = new FtpCommandExecutor(new FtpClientProvider(clientFactory), _logger);
        }
        ...
        {
            var items = _baseFtpCommand.Execute(c => c.GetListing(path));
            foreach (var item in items)
            {
                ...
                switch (item.Type)
                {
                    ...
                    case FtpFileSystemObjectType.Link:
                        _logger.Log($"Size={_baseFtpCommand.Execute(c => c.GetFileSize(item.FullName))}");
                        _baseFtpCommand.Execute(c => c.DeleteFile(item.FullName));
                        break;
                    case FtpFileSystemObjectType.Directory:
                        Clean(item.FullName);
                        _baseFtpCommand.Execute(c => c.DeleteDirectory(item.FullName));
                        break;
                }
            }
        }
        ...
        private void PushDirectory(string source, string target)
        {
            ...
            foreach (var dir in directories)
            {
                ...
                _baseFtpCommand.Execute(c => c.CreateDirectory(targetPath));
            ...
            foreach (var file in files)
            {
                ...
                _baseFtpCommand.Execute(c => c.UploadFile(file, targetPath));
            }
        }
